...
std::vector<int> RemizovKMaxInMatrixStringMPI::FindLocalMaxes(const std::vector<int> &local_data, int local_rows,
                                                              int row_size) {
...
bool RemizovKMaxInMatrixStringMPI::RunImpl() {
  ...
  MPI_Scatterv(flat_matrix.data(), send_counts.data(), send_displs.data(), MPI_INT, local_data.data(),
               my_rows * row_size, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  MPI_Gatherv(local_maxes.data(), my_rows, MPI_INT, global_maxes.data(), rows_per_process.data(), displacements.data(),
